...
public class FitnessFunction {
    ...
    public static int knightProtected(IChessMatch match, IChessPiece piece, int deltaRank) {
        ...
        int count = 0;
        if(piece.file() + 1 > 0 && piece.file() + 1 < 7 && piece.rank() + deltaRank > 0 && piece.rank() + deltaRank < 7) {
            square1 = new ChessboardSquare(piece.file(), piece.rank()  + deltaRank);
        }
        if(piece.file() - 1 > 0 && piece.file() - 1 < 7 && piece.rank() + deltaRank > 0 && piece.rank() + deltaRank < 7) {
            square2 = new ChessboardSquare(piece.file(), piece.rank()  + deltaRank);
        }
