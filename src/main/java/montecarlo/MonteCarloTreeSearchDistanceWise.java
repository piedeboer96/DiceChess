...
public class MonteCarloTreeSearchDistanceWise {
    ...
    public boolean performExpansion(MonteCarloNode node, DiceChess game, List<Opportunity> movementOpportunities) {
        ...
        List<Movement> movementsToExpand = findPrioritizedMovements(movementOpportunities, kingLocation);
        // Todo: expand using the movements above.

        for (Movement m : movementsToExpand) {
            game.register(m);
            game.switchActiveColor();
